#!/usr/bin/env python3
"""
replay_with_retry101_rev2.py — retry-replay instrument: measures the PATCHED pipeline

rev2 (2026-09-21): summary-tally fix - PASS_RETRY / FAIL_BOTH / <verdict>2 are
now counted in the FINAL OUTCOMES block and its N denominator (rev1 printed
inflated percentages, e.g. pilot showed REFUSED_DOMAIN 4 (100%) instead of
80% + FAIL_BOTH 1 (20%)). Per-row CSV output and resume logic unchanged;
existing replay_retry_results.csv rows (incl. the 5-row pilot) stay valid.
Delta: see replay_with_retry101_CHANGELOG.md.
======================================================================
...
RUN (server, chalo_chatbot project root, llama.cpp idle):
    python3 replay_with_retry101_rev2.py --limit 5     # pilot
    python3 replay_with_retry101_rev2.py               # full ~40-70 min
