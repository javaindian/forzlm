#!/usr/bin/env python3
"""ChaloSchools AI Secretary - HINT_MAP v2.1 + domain-absent fast refusal patch.

CANONICAL FILE: patch_hintmap_v2_1.py (v2.1, 2026-09-21). Supersedes
patch_hintmap_v2.py - archived under that same name in download/superseded/.
The exact v2 -> v2.1 delta is documented in patch_hintmap_CHANGELOG.md
...
What it does (all idempotent, backups kept, py_compile verified, rollback on error):
  Part A - adds the schema-verified (table[, column]) entries to HINT_MAP in
           pipeline/semantic_rules.py (v2: 76 entries; v2.1: +3 from the
           post-patch pilot). PER-ENTRY idempotent - safe to re-run over v2.
  Part B - appends DOMAIN_ABSENT_TABLES + lookup_domain_absent() to
           pipeline/semantic_rules.py (v2.1 also patches InventoryEntryMaster
           into an already-applied v2 file).
...
v2.1 changelog (2026-09-21, from the owner's --limit 10 post-patch pilot):
  new identifiers the model produced that v2 did not cover:
    FeesCollection.Branch_ID, InventoryItem_Master.ItemCategory_Name,
    InventoryItem_Master.SizeName, InventoryEntryMaster (table variant).
...
pipeline/semantic_rules.py and pipeline/orchestrator.py):
    sudo python3 patch_hintmap_v2_1.py --check    # dry run: show what would change
    sudo python3 patch_hintmap_v2_1.py            # apply
...
A_ENTRIES = [
    ...
    (("FeesCollection", "Branch_ID"),
     "FeesCollection's branch column is BranchID (no underscore)."),
    ...
    (("InventoryItem_Master", "ItemCategory_Name"),
     "InventoryItem_Master has no ItemCategory_Name. JOIN InventoryCategory_Master ic "
     "ON iim.ItemCategory_ID = ic.Id and SELECT ic.Name."),
    (("InventoryItem_Master", "SizeName"),
     "InventoryItem_Master has no SizeName/Size_Name column - item size is not tracked."),
    # --- misc ---
...
DOMAIN_ABSENT_TABLES: set = {
    ...
    "InventoryEntry", "InventoryEntryDetails", "InventoryEntryMaster", "Stock_Master",
...
def patch_semantic_rules(path: str, dry: bool) -> bool:
    ...
    changed = False
    # ---- Part A (per-entry idempotent: adds only entries not yet present) ----
    def _hint_key(tbl, col):
        return f'("{tbl}", "{col}")' if col else f'("{tbl}", None)'
    missing = [(k, t) for k, t in A_ENTRIES if _hint_key(*k) not in src]
    if not missing:
        print("[A] all HINT_MAP v2 entries present - skip")
    else:
        ...
        block = ("\n    # ── HINT_MAP v2 (2026-09-21, replay101 evidence - schema-verified; "
                 "v2.1 pilot follow-ups) ──\n")
        for (tbl, col), text in missing:
            key = _hint_key(tbl, col)
            text = text.replace('"', "'")
            block += f'    ({key}):\n        "{text}",\n'
        src = src[:close] + block + src[close:]
        changed = True
        print(f"[A] +{len(missing)} HINT_MAP entries ({len(A_ENTRIES) - len(missing)} already present)")
    # ---- Part B ----
    if MARKER_B in src:
        print("[B] DOMAIN_ABSENT_TABLES already present")
    else:
    ...
    # ---- Part B v2.1: patch InventoryEntryMaster into already-applied v2 files ----
    if '"InventoryEntryMaster"' not in src:
        mb = re.search(r"DOMAIN_ABSENT_TABLES:\s*set\s*=\s*\{", src)
        if mb:
            closeb = src.find("\n}", mb.end())
            if closeb > 0:
                src = (src[:closeb]
                       + '\n    "InventoryEntryMaster",  # v2.1: table variant seen in post-patch pilot'
                       + src[closeb:])
                changed = True
                print("[B v2.1] +InventoryEntryMaster to DOMAIN_ABSENT_TABLES")
        old_lbl = '(("InventoryEntry", "InventoryEntryDetails", "Stock_Master"),'
        new_lbl = '(("InventoryEntry", "InventoryEntryDetails", "InventoryEntryMaster", "Stock_Master"),'
        if old_lbl in src:
            src = src.replace(old_lbl, new_lbl)
            changed = True
            print("[B v2.1] +InventoryEntryMaster to inventory label group")
    if dry:
